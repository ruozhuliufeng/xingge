...
public class EntityScanner {
     ...
     * @param config 表维护配置
     */
    public EntityScanner(XingGeDataBaseCconfig config) {
        this.config = config;
    ...
    public List<Class<?>> scanEntities() throws IOException {
        ...
        if (!config.getTableMaintenance().getIncludeEntities().isEmpty()) {
            for (String entityClassName : config.getTableMaintenance().getIncludeEntities()) {
                try {
        ...
        for (String packageName : config.getTableMaintenance().getEntityPackages()) {
            List<Class<?>> packageEntities = scanPackage(packageName, scannedClasses);
            entities.addAll(packageEntities);
        }

        // 过滤排除的实体类
        entities.removeIf(entityClass -> config.getTableMaintenance().getExcludeEntities().contains(entityClass.getName()));
    ...
    private boolean isValidEntity(Class<?> entityClass) {
        ...
        // 检查是否在排除列表中
        if (config.getTableMaintenance().getExcludeEntities().contains(entityClass.getName())) {
    ...
    public String getTableName(Class<?> entityClass) {
        ...
        if (tableName.isEmpty()) {
            ...
            if (config.getTableMaintenance().isCamelCaseToUnderscore()) {
                tableName = camelCaseToUnderscore(tableName);
            }
        }

        // 添加前缀和后缀
        if (config.getTableMaintenance().getTablePrefix() != null && !config.getTableMaintenance().getTablePrefix().isEmpty()) {
            tableName = config.getTableMaintenance().getTablePrefix() + tableName;
        }
        if (config.getTableMaintenance().getTableSuffix() != null && !config.getTableMaintenance().getTableSuffix().isEmpty()) {
            tableName = tableName + config.getTableMaintenance().getTableSuffix();
        }
