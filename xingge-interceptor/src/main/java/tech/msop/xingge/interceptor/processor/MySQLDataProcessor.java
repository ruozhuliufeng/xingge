...
public class MySQLDataProcessor implements DataProcessor {
    ...
    private static final String PROCESSOR_TYPE = "mysql";
    ...
    public boolean validateConfig(Map<String, Object> config) {
        String tableName = (String) config.get(CONFIG_TABLE_NAME);
        if (tableName == null || tableName.trim().isEmpty()) {
            log.error("MySQL处理器配置验证失败：缺少表名");
            return false;
        }
        
        if (jdbcTemplate == null) {
            log.error("MySQL处理器配置验证失败：未找到JdbcTemplate");
            return false;
        }
    ...
    private void insertSingle(InterceptData data, String tableName, Map<String, Object> config) {
        ...
        log.debug("成功插入拦截数据到MySQL表: {}", tableName);
    }
    ...
    private void flushBatch(String tableName, Map<String, Object> config) {
        ...
        log.debug("成功批量插入{}条拦截数据到MySQL表: {}", batchData.size(), tableName);
    ...
    private void createTableIfNotExists(String tableName) {
        try {
            ...
            log.info("成功创建MySQL拦截数据表: {}", tableName);
        } catch (Exception e) {
            log.debug("MySQL表{}可能已存在，跳过创建: {}", tableName, e.getMessage());
        }
    }
    ...
    private String buildCreateTableSQL(String tableName) {
        return "CREATE TABLE IF NOT EXISTS " + tableName + " (" +
                ...
                "INDEX idx_intercept_scope (intercept_scope), " +
