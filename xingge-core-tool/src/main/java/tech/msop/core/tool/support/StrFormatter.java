...
public class StrFormatter {
	...
	public static String format(final String strPattern, final Object... argArray) {
		...
		for (int argIndex = 0; argIndex < argArray.length; argIndex++) {
			delimIndex = strPattern.indexOf(StringConstant.EMPTY_JSON, handledPosition);
			/**
			 * 剩余部分无占位符
			 */
			if (delimIndex == -1) {
				 ...
				 * 转义符
				 */
				if (delimIndex > 0 && strPattern.charAt(delimIndex - 1) == CharConstant.BACK_SLASH) {
					/**
					 * 双转义符
					 */
					if (delimIndex > 1 && strPattern.charAt(delimIndex - 2) == CharConstant.BACK_SLASH) {
						...
						argIndex--;
						sbuf.append(strPattern, handledPosition, delimIndex - 1);
						sbuf.append(CharConstant.LEFT_BRACE);
